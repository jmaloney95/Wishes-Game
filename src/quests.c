...
// Vanilla-color text palette: WoT's dark UI theme repaints gStandardMenuPalette
// (BG palette 15) with light text colors, which are unreadable on this menu's
// parchment background. Every other menu reloads palette 15 on entry, so
// overriding it here does not leak into the rest of the UI.
static const u16 sQuestMenuTextPals[] =
        INCBIN_U16("graphics/quest_menu/text.gbapal");
	...
	switch (gMain.state)
	{
		...
		case 15:
			...
			gTasks[taskId].data[0] = ListMenuInit(&gMultiuseListMenuTemplate,
			                                      ...
			                                      sListMenuState.row);
			ListMenuSetCursorWrap(gTasks[taskId].data[0], TRUE);
			gMain.state++;
			break;
		case 16:
...
static bool8 LoadGraphics(void)
{
	switch (sStateDataPtr->data[0])
	{
		...
		case 2:
			...
			LoadPalette(sQuestMenuTextPals, BG_PLTT_ID(15), sizeof(sQuestMenuTextPals));
			sStateDataPtr->data[0]++;
			break;
		case 3:
			sStateDataPtr->data[0]++;
...
static void QuestMenu_CreateSprite(u16 itemId, u8 idx, u8 spriteType)
{
	...
	if (ptr[idx] == 0xFF)
	{
		...
		if (spriteId != MAX_SPRITES && spriteId != 0xFF)
		{
...
static void RestoreSavedScrollAndRow(s16 *data)
{
	data[0] = ListMenuInit(&gMultiuseListMenuTemplate,
	                       ...
	                       sListMenuState.storedRowPosition);
	ListMenuSetCursorWrap(data[0], TRUE);
}
static void ResetCursorToTop(s16 *data)
{
	sListMenuState.row = 0;
